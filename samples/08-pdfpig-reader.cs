...
// IOcrClient). But most PDFs already carry a digital text layer — paying an OCR engine to re-read text
...
//     seam 2  IOcrClient + OcrPolicy -> WHEN to OCR (Never / FallbackForEmptyPages / AllPages), any engine
//
// The reader depends on IOcrClient ONLY — zero IChatClient. The vision-LLM path is just one IOcrClient
...
// and the reader below is untouched — that is the whole point of composing IOcrClient.
...
// would route to the injected IOcrClient.
...
// --- 3) AllPages: hand the whole document to the IOcrClient (document-native archetype). -------------
var ocrAll = new PdfPigReader(ocr, OcrPolicy.AllPages);
IngestionDocument ocrDoc = await Read(ocrAll, pdf);
Console.WriteLine($"[AllPages]              whole doc -> IOcrClient -> {ocrDoc.Sections.Count} pages, {Elements(ocrDoc)} elements, OCR calls: {ocrAll.OcrCalls}");
...
Console.WriteLine($"\nBoth readers feed the SAME chunker: [Never] document -> {chunks} chunks. One pipeline, two readers, one IOcrClient seam.");
return 0;
